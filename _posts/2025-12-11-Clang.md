...
**From a Makefile project** (most STM32CubeIDE / bare-metal projects), use [compiledb](https://github.com/nickdiego/compiledb):
...
{
    "clangd.arguments": [
        # Allow clangd to query the cross-compiler for system include paths
        # (stdint.h, stdbool.h, etc. from the ARM toolchain, not the host)
...
<h3 id="clang-format" style="font-weight: bold;">Code Formatting with .clang-format</h3>

clangd uses `.clang-format` for format-on-save. Place it in the project root:
...
<h3 id="practical-example" style="font-weight: bold;">Practical Example: STM32 Bare-Metal Project</h3>
...
<h3 id="troubleshooting" style="font-weight: bold;">Troubleshooting</h3>

**Issue 1: Red errors everywhere**

```
error: 'stm32f4xx.h' file not found
```

clangd cannot find include paths because `compile_commands.json` is missing. Run `compiledb make` to generate it.

**Issue 2: Wrong type sizes or missing macros**

```
error: use of undeclared identifier '__IO'
```

The target triple is not set. Add `--target=arm-none-eabi` to the `Add:` section of `.clangd`.

**Issue 3: `unknown argument` warnings flood the log**

```
warning: unknown argument: '-mfpu=fpv4-sp-d16'
```

GCC-specific flags are leaking into clangd. Add them to the `Remove:` section of `.clangd`.
...
| `.clang-format` | Code style rules for format-on-save |
