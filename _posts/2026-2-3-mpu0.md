...
![L1](/assets/img/2026-2-3-mpu0/L1.png)
...
![Memory-mapp](/assets/img/2026-2-3-mpu0/img3.png)
...
- `Invalidate`: Removing entries from the cache. (like delete operation, remove the data in cache)

- `Clean`: Writing back dirty cache lines to the main memory. (AKA flush term, write back the data from cache to memories)
...
![Write-Through](/assets/img/2026-2-3-mpu0/wt.png)
...
<h5 id="Example: Write-Through Cache Policy" style="font-weight: bold;">Example: Write-Through Cache Policy</h5>

```markdown
Assume the processor wants to write the value 20 to memory address 0x20001000.
...
The address is already present in the cache (cache hit).
...
Both the cache and main memory are updated immediately, so they always contain the same value.
```
...
![Write-back](/assets/img/2026-2-3-mpu0/wb.png)
...
<h5 id="Example: Write-Back Cache Policy" style="font-weight: bold;">Example: Write-Back Cache Policy</h5>
...
The cache contains the updated value, while the main memory still holds the old value.

Later, when this cache line is evicted (or during a cache clean operation), the cache writes the updated value back to main memory.

Cache eviction (or cache clean)

Cache (20) -> Main Memory (20)
Dirty bit = 0
...
- [Cache on ST F7 and H7](https://www.st.com/resource/en/application_note/an4839-level-1-cache-on-stm32f7-series-and-stm32h7-series-stmicroelectronics.pdf)
- [Guideline on how to use Cache]:https://community.st.com/stm32-mcus-60/how-to-use-the-memory-protection-unit-to-manage-cache-coherency-on-high-performance-mcus-159722
